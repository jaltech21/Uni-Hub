/**
 * AdminCoursesScreen
 * View, create, toggle-active, and delete courses.
 */
...
export default function AdminCoursesScreen() {
  ...
  const handleCreate = async () => {
    ...
    const deptId = parseInt(newDeptId, 10);
    if (!deptId) {
      Alert.alert("Validation", "A valid Department ID is required");
      return;
    }
    try {
      ...
      Alert.alert("Error", e.message || "Could not create course");
    }
  };

  const handleToggle = async (course: Course) => {
    try {
      await adminService.toggleCourseActive(course.id);
      load();
    } catch (e: any) {
      Alert.alert("Error", e.message || "Could not toggle course");
    }
  };
  ...
  const renderItem = ({ item }: { item: Course }) => (
    <View style={styles.row}>
      <View style={styles.rowBody}>
        ...
        >
          {item.active ? "Active" : "Inactive"} · Dept #{item.department_id}
        </Text>
      </View>
      <View style={styles.actions}>
        <TouchableOpacity
          style={[
            styles.btn,
            { backgroundColor: item.active ? colors.warning[50] : colors.success[50] },
          ]}
        ...
        >
          <Text
            style={[
              styles.btnText,
              { color: item.active ? colors.warning[700] : colors.success[700] },
            ]}
          >
